//! Coordinate spaces.
//!
//! The app juggles three of them, and mixing them up is the single easiest way
//! to break multi-monitor support, so they get distinct types:
//!
//! * **virtual device px** — the whole desktop in physical pixels. This is what
//!   the window system reports for monitor placement.
//! * **monitor logical px** — one monitor, origin at its top-left, divided by
//!   that monitor's scale factor. This is what egui draws in, and what
//!   measurements are reported in.
//! * **monitor image px** — one monitor's captured screenshot, in physical
//!   pixels. The edge map and region map live here.
//!
//! A monitor's scale factor converts between the last two. Because each monitor
//! carries its own, a 2x laptop panel beside a 1x external display measures
...
pub struct MonitorGeometry {
    /// Human-readable output name, e.g. `DP-1`. Used for diagnostics.
    pub name: String,
...
impl MonitorGeometry {
    /// Guards against a window system reporting a nonsensical scale factor,
    /// which would otherwise divide measurements by zero.
    ...
    ///
    /// Part of the virtual-desktop coordinate seam kept for cross-monitor
    /// features (dragging a measurement from one screen onto another), which
...
///
/// Returns `None` for an empty monitor list. Part of the cross-monitor seam.
#[allow(dead_code)]
...
mod tests {
    ...
    fn each_monitor_applies_its_own_scale() {
        // The mixed-DPI case the previous single-overlay design got wrong: a
        // 100px logical span is 200 device px on the laptop panel and 100 on
