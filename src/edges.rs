...
//!
//! Hand-rolled instead of binding OpenCV: the ruler only needs one fixed
//! pipeline (blur -> Sobel -> non-maximum suppression -> hysteresis), and
//! dropping the dependency is what makes a single self-contained binary
//! practical on all three platforms.

use crate::image::Gray8;

/// Sensitivity is a user-facing 0..100 dial; these anchor the Canny
/// hysteresis thresholds it maps onto. Carried over from the Python version so
/// existing muscle memory for the slider still holds.
const LOW_AT_100: f32 = 5.0;
const HIGH_AT_100: f32 = 25.0;
const LOW_AT_0: f32 = 80.0;
const HIGH_AT_0: f32 = 220.0;

/// Default dial position, matching the previous implementation's defaults of
...
impl EdgeMap {
    pub fn new(width: usize, height: usize, data: Vec<bool>) -> Option<Self> {
    ...
    #[inline]
    pub fn any_in_row(&self, y: usize, x0: usize, x1: usize) -> bool {
        if y >= self.height || x0 > x1 || x0 >= self.width {
            return false;
        }
        let x1 = x1.min(self.width - 1);
        let start = y * self.width + x0;
        self.data[start..=start + (x1 - x0)].iter().any(|v| *v)
    }

    /// True when any pixel in the inclusive row span of a single column is an edge.
    #[inline]
    pub fn any_in_column(&self, x: usize, y0: usize, y1: usize) -> bool {
        if x >= self.width || y0 > y1 || y0 >= self.height {
            return false;
        }
        let y1 = y1.min(self.height - 1);
        (y0..=y1).any(|y| self.data[y * self.width + x])
    }
}
...
pub fn sensitivity_to_thresholds(sensitivity: f32) -> (u16, u16) {
    ...
    let high = if high <= low { (low + 1).min(255) } else { high };
    (low, high)
}
...
/// blank map rather than an error, so a degenerate monitor never aborts start-up.
pub fn canny(gray: &Gray8, low: u16, high: u16) -> EdgeMap {
    let (w, h) = (gray.width(), gray.height());
    if w < 3 || h < 3 {
        return EdgeMap::blank(w, h);
    }

    let (low, high) = if high <= low { (low, low + 1) } else { (low, high) };
...
fn gaussian_blur_5(gray: &Gray8) -> Vec<u16> {
...
fn quantise_direction(gx: i32, gy: i32) -> Dir {
    // tan(22.5 deg) ~= 0.414, tan(67.5 deg) ~= 2.414; comparing with integer
    // ratios avoids an atan2 per pixel.
...
fn hysteresis(thin: &[u16], w: usize, h: usize, low: u16, high: u16) -> EdgeMap {
    ...
    EdgeMap::new(w, h, out).expect("hysteresis preserves dimensions")
}
...
mod tests {
    ...
    fn gray_from(width: usize, height: usize, f: impl Fn(usize, usize) -> u8) -> Gray8 {
        ...
        Gray8::from_raw(width, height, pixels).expect("valid buffer")
    }
    ...
    fn sensitivity_clamps_out_of_range_input() {
        assert_eq!(sensitivity_to_thresholds(-40.0), sensitivity_to_thresholds(0.0));
        assert_eq!(sensitivity_to_thresholds(140.0), sensitivity_to_thresholds(100.0));
    }
    ...
    fn default_sensitivity_matches_legacy_cli_thresholds() {
        ...
        assert!((recovered - DEFAULT_SENSITIVITY).abs() < 1.0, "got {recovered}");
    }
    ...
    fn detects_a_vertical_step_edge_and_thins_it() {
        ...
        for y in 6..26 {
            let hits: Vec<usize> = (0..32).filter(|x| edges.at(*x, y)).collect();
    ...
    fn detects_a_horizontal_step_edge() {
        ...
        let edges = canny(&img, 5, 25);
        for x in 6..26 {
            let hits: Vec<usize> = (0..32).filter(|y| edges.at(x, *y)).collect();
