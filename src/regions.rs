...
/// 3x3 structuring element, matching the previous OpenCV `MORPH_CLOSE` call.
...
impl RegionMap {
    ...
    pub fn build(edges: &EdgeMap) -> Self {
        ...
        for seed in 0..w * h {
            ...
            // Flood fill with 4-connectivity, mirroring the previous
            // `connectedComponentsWithStats(..., connectivity=4)` behaviour.
    ...
    fn label_near(&self, x: usize, y: usize, max_radius: usize) -> u32 {
    ...
    pub fn container_at(&self, x: usize, y: usize) -> Option<RegionStats> {
        ...
        let screen_area = self.width as f64 * self.height as f64;
...
mod tests {
    ...
    fn boxed_map(gap: Option<(usize, usize)>) -> EdgeMap {
        const W: usize = 24;
        const H: usize = 18;
        let (left, top, right, bottom) = (4usize, 3usize, 19usize, 14usize);

        let mut data = vec![false; W * H];
        for x in left..=right {
            data[top * W + x] = true;
            data[bottom * W + x] = true;
        }
        for y in top..=bottom {
            data[y * W + left] = true;
            data[y * W + right] = true;
    ...
    fn labels_a_boxed_interior_as_its_own_region() {
        ...
        assert!(regions.region_count() >= 2, "got {}", regions.region_count());

        let inner = regions.container_at(12, 8).expect("interior is a container");
        assert_eq!(
            inner,
            RegionStats {
                x: 5,
    ...
    fn a_wide_gap_is_not_sealed_and_the_interior_merges_outward() {
        ...
        let mut open = boxed_map(None);
        let mut data = open.as_slice().to_vec();
        for y in 7..=10 {
            data[y * 24 + 19] = false;
        }
        open = EdgeMap::new(24, 18, data).expect("valid map");
    ...
    fn cursor_on_a_barrier_snaps_to_the_nearest_region() {
        ...
        let resolved = regions.container_at(12, 3).expect("barrier resolves outward");
